...
str_buffer = "Medulla 3p Bot Running"
...
def main():
    ...
    while True:
        ...
        t = Thread(target=hitter.hold_key, args=('w', 17.47))
        threads.append(t)

        arr1 , arr2, arr3 = ['a','d','d'],[0.231,0.391,0.435],[2.915,6.62,11.181]
        ...
        #Enter Next Fight
        feinter.hold_key('w', random.uniform(0.8, 0.9))
        blader.hold_key('w', random.uniform(0.8, 0.9))
        hitter.hold_key('w', random.uniform(0.8, 0.9))

        feinter.wait(.5)

        feinter.hold_key('w', random.uniform(1.0, 1.1))
        blader.hold_key('w', random.uniform(1.0, 1.1))
        hitter.hold_key('w', random.uniform(1.0, 1.1))
        ...
        while inFight:
            ...
            if(battle_round >= 4):
                ...
                logout_failsafe([feinter, hitter, blader])
                spawn_program_and_die(['python', 'farm_Medulla.py',str(ROUND_COUNT), str(failed_runs), str(timeout_fails),str(PROGRAM_START_TIME)]) 
                break

            random.shuffle(user_order)

            user_order[0][0].riddler_attack(wizard_type = user_order[0][1], boss_pos = boss_pos, round_num = battle_round)
            user_order[1][0].riddler_attack(wizard_type = user_order[1][1], boss_pos = boss_pos, round_num = battle_round)
            user_order[2][0].riddler_attack(wizard_type = user_order[2][1], boss_pos = boss_pos, round_num = battle_round)
        ...
        t = Thread(target=hitter.hold_key, args=('w', 14.124))
        threads.append(t)

        arr1 , arr2, arr3 = ['a','a','d'],[0.195,0.29,0.21],[2.577,5.329,11.874]
        ...
        t = Thread(target=blader.hold_key, args=('w', 14.024))
        threads.append(t)

        arr1 , arr2, arr3 = ['d','a','d'],[0.18,0.39,0.29],[0.121,5.017,10.47]
        ...
        t = Thread(target=feinter.hold_key, args=('w', 14.288))
        threads.append(t)

        arr1 , arr2, arr3 = ['d','a','d'],[0.35,0.351,0.35],[0.183,4.906,11.336]
        ...
        """ Run into battle """
        feinter.hold_key('w', random.uniform(1.0, 1.0))
        blader.hold_key('w', random.uniform(1.0, 1.0))
        hitter.hold_key('w', random.uniform(1.5, 1.5))
        ...
        while inFight:
            ...
            user_order[0][0].medulla_attack(wizard_type = user_order[0][1], boss_pos = boss_pos, round_num = battle_round)
            user_order[1][0].medulla_attack(wizard_type = user_order[1][1], boss_pos = boss_pos, round_num = battle_round)
            user_order[2][0].medulla_attack(wizard_type = user_order[2][1], boss_pos = boss_pos, round_num = battle_round)
